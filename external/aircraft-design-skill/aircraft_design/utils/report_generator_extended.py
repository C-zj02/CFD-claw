...
为了更直观地展示设计方案，并为后续的气动计算准备几何输入，本流程生成参数化几何、OpenVSP 脚本和展示资产。除非执行记录明确给出 OpenVSP 版本、命令和成功退出状态，否则这些文件不代表已运行 OpenVSP 或完成几何干涉验证。
...
合理的重心范围通常设定在平均气动弦长（MAC）的 15% 至 35% 之间。当前结果只包含总体设计阶段的静稳定筛查；完整装载组合、燃油消耗过程和操纵权限仍需专项重心包线分析。
...
### 6.3 任务航程与航时范围
任务燃油闭合使用 Breguet 关系和分段任务模型：
$$ R = \\frac{{V}}{{C}} \\frac{{L}}{{D}} \ln(\\frac{{W_{{initial}}}}{{W_{{final}}}}) $$
当前模型按输入的 {req.range_m / 1000} km 总任务航程分配爬升、巡航和下降距离，并据此闭合任务燃油与重量。该数值是规定任务剖面的评估距离，不是独立预测的最大航程；模型也没有独立预测最大燃油限制巡航航时。航程能力、巡航航时和储备合规性仍应按交付判定中的模型覆盖范围解释。
...
这些起降数值必须与当前 revision 的用户约束和阻断裕度逐项比较；本章节本身不判定机场适用性或整机可交付性。
...
### 13.1 结论
本报告记录当前总体设计迭代得到的候选参数，但不独立判定整机最终可行。由项目编排器运行时，最终结论以其追加的“需求范围与交付判定”、阻断约束裕度和专项验证缺口为准；直接 upstream CLI 输出仍是低层诊断，不构成交付结论。
1. **总体参数**：当前重量闭合结果为 {aircraft.mtow_kg:.1f} kg；是否满足最大起飞重量要求取决于当前 revision 的上限及其裕度。
2. **气动性能**：升阻比和最大升力系数来自当前低阶模型与假设，仍需高保真气动验证。
3. **飞行性能**：任务距离闭合、速度和高度筛查不能替代独立最大航程、最大航时和全包线能力验证。
4. **三维设计**：参数化模型、OBJ 和 OpenVSP 脚本是展示及后续分析输入，不证明 OpenVSP 已执行、几何无干涉或结构可制造。
